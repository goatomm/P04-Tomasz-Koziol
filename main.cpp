...
vector<int> read_data(istream& in, int n)// funkcja zczytujaca wektor
{
    vector<int> ret;//tworzenie pomocniczego wektora ret

    for(int i = 0; i < n || n < 0; i++) // petla zaczytujaca elementy wektora
    {
        int x;
        if (in >> x) // wczytywanie wlementow do wekotra
            ret.push_back(x);i<n // zwiekszanie konca wektora elementem x
            break;
    }
    return ret;
}
